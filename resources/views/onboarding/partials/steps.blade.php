    ...
    @foreach ($steps as $num => $label)
        ...
        <div style="display:flex;align-items:center;flex:{{ $last ? '0 0 auto' : '1' }};">
            <div style="display:flex;flex-direction:column;align-items:center;">
                <div style="width:1.5rem;height:1.5rem;flex-shrink:0;display:flex;align-items:center;justify-content:center;font-size:0.6rem;font-weight:700;
                    {{ $done   ? 'background:#000;color:#fff;'                                          : '' }}
                    {{ $active ? 'background:#5e17eb;color:#fff;'                                       : '' }}
                    {{ !$done && !$active ? 'background:#eee;color:#999;border:1px solid #ddd;' : '' }}
                ">
                    @if ($done)
                ...
                <span style="font-size:0.55rem;font-weight:700;margin-top:0.25rem;white-space:nowrap;letter-spacing:0.05em;text-transform:uppercase;
                    {{ $active ? 'color:#5e17eb;' : '' }}
                    {{ $done   ? 'color:#000;'    : '' }}
                    {{ !$done && !$active ? 'color:#bbb;' : '' }}
                ">{{ $label }}</span>
            </div>
            @if (!$last)
                <div style="flex:1;height:1.5px;background:{{ $done ? '#000' : '#ddd' }};margin:0 0.5rem;margin-bottom:1rem;"></div>
            @endif
        </div>
    @endforeach
</div>
