...
<body>
    ...
    <table style="width:100%;border-collapse:collapse;">
        <tr>
            <td style="vertical-align:top;width:50%;">
                <div><span class="meta-label">Quote For</span><span class="meta-value">{{ optional($quotation->customer)->name ?? $quotation->customer_name }}</span></div>
                @if ($quotation->customer_email)
                    <div><span class="meta-label">Email</span><span class="meta-value">{{ $quotation->customer_email }}</span></div>
                @endif
                @if ($quotation->customer_address)
                    <div><span class="meta-label">Address</span><span class="meta-value" style="white-space:pre-line;">{{ $quotation->customer_address }}</span></div>
                @endif
            </td>
            <td style="vertical-align:top;width:50%;">
                <div class="doc-title">QUOTATION</div>
                <div style="text-align:right;font-size:7.5pt;">Quotation No: <strong>{{ $quotation->quotation_number }}</strong></div>
                <div style="text-align:right;font-size:7.5pt;">Date: <strong>{{ $quotation->quotation_date->format('Y-m-d') }}</strong></div>
                @if ($quotation->expiry_date)
                    <div style="text-align:right;font-size:7.5pt;">Valid Until: <strong>{{ $quotation->expiry_date->format('Y-m-d') }}</strong></div>
                @endif
                <div style="text-align:right;margin-top:4px;"><span style="font-weight:bold;text-transform:uppercase;border:1px solid #000;padding:1px 5px;font-size:6pt;letter-spacing:0.08em;">{{ $quotation->status }}</span></div>
            </td>
        </tr>
    </table>
        ...
        <div class="info-section">
            ...
            <table class="payment-table">
                <tr>
                    ...
                    @endif
                    <td><span class="lbl">Reference</span>{{ $quotation->quotation_number }}</td>
                </tr>
            </table>
        </div>
    @endif
    ...
    @if ($quotation->notes)
        <div class="info-section">
            <div class="section-header">Notes / Terms</div>
    ...
    <div class="total-due-wrapper">
        ...
        <div class="footer-text">
            ...
            @if ($quotation->expiry_date)
                &mdash; This quotation is valid until {{ $quotation->expiry_date->format('d M Y') }}.
            @endif
        </div>
    </div>
