...
@section('title', $company->registered_name . ' — Edit Purchase Order ' . $purchaseOrder->po_number)
...
@section('content')
    <div class="co-wrap">
        ...
        <div class="co-body">
            ...
            <main class="co-main">
                ...
                <form method="POST" action="{{ route('companies.purchase-orders.update', [$company, $purchaseOrder]) }}" class="cn-form" id="po-form">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="cn-section">
                        <p class="cn-section-title">Purchase Order Details</p>
                        <div class="cn-grid three">
                            <div class="cn-field">
                                <label>PO # <span style="color:#dc2626;">*</span></label>
                                <input type="text" name="po_number" value="{{ old('po_number', $purchaseOrder->po_number) }}" required>
                                @error('po_number')<p class="form-error">{{ $message }}</p>@enderror
                            </div>
                            <div class="cn-field">
                                <label>Order Date <span style="color:#dc2626;">*</span></label>
                        ...
                        <div class="cn-grid" style="margin-top:0.75rem;">
                            <div class="cn-field">
                                <label>Status</label>
                                <select name="status">
                                    <option value="draft" {{ old('status', $purchaseOrder->status->value) === 'draft' ? 'selected' : '' }}>Draft</option>
                                    <option value="sent"  {{ old('status', $purchaseOrder->status->value) === 'sent'  ? 'selected' : '' }}>Sent</option>
                                </select>
                            </div>
                            <div class="cn-field">
                                <label>Currency</label>
                                <input type="text" name="currency" value="{{ old('currency', $purchaseOrder->currency ?? 'ZAR') }}" maxlength="3">
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="cn-field">
                                ...
                                <select name="supplier_id" id="supplier_id" required>
                                    ...
                                    @foreach ($suppliers as $supplier)
                                        <option value="{{ $supplier->id }}" {{ (int) old('supplier_id', $purchaseOrder->supplier_id) === $supplier->id ? 'selected' : '' }}>
                    ...
                    <div class="cn-section">
                        ...
                        <textarea name="notes" id="notes" rows="3" style="width:100%;max-width:600px;border:1px solid #d3e2f5;padding:0.4rem 0.6rem;font-size:0.8rem;font-family:inherit;">{{ old('notes', $purchaseOrder->notes) }}</textarea>
                    </div>

                    <div style="display:flex;gap:1rem;align-items:center;">
                        <button type="submit" class="btn-submit">Save Changes</button>
                        <a href="{{ route('companies.purchase-orders.show', [$company, $purchaseOrder]) }}" style="font-size:0.78rem;color:#5a7186;text-decoration:none;">Cancel</a>
                    </div>
                </form>
    ...
    @php
        $oldItemsForJs = old('items', $purchaseOrder->items->map(fn ($item) => [
