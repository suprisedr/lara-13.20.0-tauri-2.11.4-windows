    ...
    <table style="width:100%;border-collapse:collapse;">
        <tr>
            <td style="vertical-align:top;width:50%;">
                <div><span class="meta-label">Supplier</span><span class="meta-value">{{ optional($purchaseOrder->supplier)->name ?? '—' }}</span></div>
                @if (optional($purchaseOrder->supplier)->email)
                    <div><span class="meta-label">Email</span><span class="meta-value">{{ $purchaseOrder->supplier->email }}</span></div>
                @endif
                @if (optional($purchaseOrder->supplier)->phone)
                    <div><span class="meta-label">Phone</span><span class="meta-value">{{ $purchaseOrder->supplier->phone }}</span></div>
                @endif
                @if (optional($purchaseOrder->supplier)->address)
                    <div><span class="meta-label">Address</span><span class="meta-value" style="white-space:pre-line;">{{ $purchaseOrder->supplier->address }}</span></div>
                @endif
            </td>
            <td style="vertical-align:top;width:50%;">
                <div class="doc-title">PURCHASE ORDER</div>
                <div style="text-align:right;font-size:7.5pt;">PO No: <strong>{{ $purchaseOrder->po_number }}</strong></div>
                <div style="text-align:right;font-size:7.5pt;">Date: <strong>{{ $purchaseOrder->order_date->format('Y-m-d') }}</strong></div>
                @if ($purchaseOrder->expected_delivery_date)
                    <div style="text-align:right;font-size:7.5pt;">Expected Delivery: <strong>{{ $purchaseOrder->expected_delivery_date->format('Y-m-d') }}</strong></div>
                @endif
                <div style="text-align:right;margin-top:4px;"><span style="font-weight:bold;text-transform:uppercase;border:1px solid #000;padding:1px 5px;font-size:6pt;letter-spacing:0.08em;">{{ $purchaseOrder->status->value }}</span></div>
            </td>
        </tr>
    </table>
