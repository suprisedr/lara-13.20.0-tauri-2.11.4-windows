...
@section('title', $company->registered_name . ' — New Invoice')
...
@push('styles')
    ...
    <style>
        ...
        .cust-hit { padding:0.5rem 0.8rem; cursor:pointer; font-size:0.8rem; border-bottom:1px solid #f3f0ff; }
        .cust-hit:hover { background:#faf5ff; }
        ...
        .payment-grid { display:flex; flex-wrap:wrap; gap:1.25rem 2.5rem; padding:0.75rem 0; }
        .payment-grid .pay-label { font-size:0.6rem; font-weight:700; letter-spacing:0.07em; text-transform:uppercase; color:#555; margin:0 0 0.15rem; }
        .payment-grid .pay-value { font-size:0.82rem; font-weight:600; color:#000; margin:0; }
        .payment-grid .pay-mono { font-family:'Courier New',monospace; }

        .stock-hint { font-size:0.62rem; color:#6b7280; margin-top:1px; }
    </style>
@endpush
    ...
    <div class="co-wrap">
        ...
        <div class="co-body">
            ...
            <main class="co-main">
                ...
                <form method="POST" action="{{ route('companies.invoices.store', $company) }}" class="cn-form" id="invoice-form">
                    @csrf
                    ...
                    <div class="cn-section">
                        <p class="cn-section-title">Invoice Details</p>
                    ...
                    {{-- Customer --}}
                    <div class="cn-section">
                        <p class="cn-section-title">Bill To</p>
                        <div class="cn-grid">
                            <div class="cn-field">
                        ...
                        @else
                            ...
                            <table class="cn-totals" style="margin-top:1rem;">
                                <tbody>
                                    ...
                                    <tr class="total-row"><td>Total Due</td><td class="r" id="summary-total">R 0.00</td></tr>
                                </tbody>
                            </table>
                        @endif
                    </div>
                    ...
                    @if ($company->bank_name || $company->bank_account_number)
                        <div class="cn-section">
                            <p class="cn-section-title">Payment Details</p>
                            <div class="payment-grid">
                                @if ($company->bank_name)
                                    <div>
                                        <p class="pay-label">Bank</p>
                                        <p class="pay-value">{{ $company->bank_name }}</p>
                                    </div>
                                @endif
                                @if ($company->bank_account_number)
                                    <div>
                                        <p class="pay-label">Account Number</p>
                                        <p class="pay-value pay-mono">{{ $company->bank_account_number }}</p>
                                    </div>
                                @endif
                                @if ($company->bank_account_type)
                                    <div>
                                        <p class="pay-label">Account Type</p>
                                        <p class="pay-value">{{ ucfirst($company->bank_account_type) }}</p>
                                    </div>
                                @endif
                                @if ($company->bank_branch_code)
                                    <div>
                                        <p class="pay-label">Branch Code</p>
                                        <p class="pay-value pay-mono">{{ $company->bank_branch_code }}</p>
                                    </div>
                                @endif
                                <div>
                                    <p class="pay-label">Reference</p>
                                    <p class="pay-value pay-mono" id="payment-ref" style="color:#5e17eb;">{{ old('invoice_number', $nextNumber) }}</p>
                                </div>
                            </div>
                        </div>
                    @endif
                    ...
                    <div style="display:flex;gap:1rem;align-items:center;">
                        ...
                        <a href="{{ route('companies.invoices.index', $company) }}" style="font-size:0.78rem;color:#6b7280;text-decoration:none;">Cancel</a>
                    </div>
                </form>
        ...
        (function () {
            ...
            function showDropdown(hits) {
                ...
                dropdown.innerHTML = hits.map(h => `
                    ...
                    `<div style="padding:0.4rem 0.85rem;font-size:0.7rem;color:#9ca3af;background:#f9fafb;"><em>or continue typing to add manually</em></div>`;
