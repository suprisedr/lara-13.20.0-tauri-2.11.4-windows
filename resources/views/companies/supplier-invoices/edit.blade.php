...
@section('title', $company->registered_name . ' — Edit Supplier Invoice ' . $supplierInvoice->invoice_number)
...
@section('content')
    <div class="co-wrap">
        ...
        <div class="co-body">
            ...
            <main class="co-main">
                ...
                <form method="POST" action="{{ route('companies.supplier-invoices.update', [$company, $supplierInvoice]) }}" class="cn-form" id="sinv-form">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="cn-section">
                        ...
                        <div class="cn-grid three">
                            <div class="cn-field">
                                ...
                                <input type="text" name="invoice_number" value="{{ old('invoice_number', $supplierInvoice->invoice_number) }}" required>
                                @error('invoice_number')<p class="form-error">{{ $message }}</p>@enderror
                            </div>
                            <div class="cn-field">
                                <label>Invoice Date <span style="color:#dc2626;">*</span></label>
                                <input type="date" name="invoice_date" value="{{ old('invoice_date', $supplierInvoice->invoice_date->format('Y-m-d')) }}" required>
                                @error('invoice_date')<p class="form-error">{{ $message }}</p>@enderror
                            </div>
                            <div class="cn-field">
                                <label>Due Date</label>
                                <input type="date" name="due_date" value="{{ old('due_date', $supplierInvoice->due_date?->format('Y-m-d')) }}">
                        ...
                        <div class="cn-grid" style="margin-top:0.75rem;">
                            <div class="cn-field">
                                <label>Status</label>
                                <select name="status">
                                    <option value="draft"    {{ old('status', $supplierInvoice->status->value) === 'draft'    ? 'selected' : '' }}>Draft</option>
                                    <option value="pending"  {{ old('status', $supplierInvoice->status->value) === 'pending'  ? 'selected' : '' }}>Pending</option>
                                    <option value="approved" {{ old('status', $supplierInvoice->status->value) === 'approved' ? 'selected' : '' }}>Approved</option>
                                </select>
                            </div>
                            <div class="cn-field">
                                <label>Currency</label>
                                <input type="text" name="currency" value="{{ old('currency', $supplierInvoice->currency ?? 'ZAR') }}" maxlength="3">
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="cn-field">
                                ...
                                <select name="supplier_id" id="supplier_id" required>
                                    ...
                                    @foreach ($suppliers as $supplier)
                                        <option value="{{ $supplier->id }}" {{ (int) old('supplier_id', $supplierInvoice->supplier_id) === $supplier->id ? 'selected' : '' }}>
                    ...
                    <div class="cn-section">
                        ...
                        <textarea name="notes" id="notes" rows="3" style="width:100%;max-width:600px;border:1px solid #d3e2f5;padding:0.4rem 0.6rem;font-size:0.8rem;font-family:inherit;">{{ old('notes', $supplierInvoice->notes) }}</textarea>
                    </div>

                    <div style="display:flex;gap:1rem;align-items:center;">
                        <button type="submit" class="btn-submit">Save Changes</button>
                        <a href="{{ route('companies.supplier-invoices.show', [$company, $supplierInvoice]) }}" style="font-size:0.78rem;color:#5a7186;text-decoration:none;">Cancel</a>
                    </div>
                </form>
    ...
    @php
        $oldItemsForJs = old('items', $supplierInvoice->items->map(fn ($item) => [
    ...
    <script>
        ...
        function addLineItem(data = null) {
            ...
            if (data?.inventory_item_id) {
                ...
                if (item) {
                    // Set search field but don't overwrite description/price from data
                    document.getElementById(`item-search-${idx}`).value = item.name;
                    document.getElementById(`item-id-${idx}`).value = item.id;
                }
            }
