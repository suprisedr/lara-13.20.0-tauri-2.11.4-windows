...
@section('title', $company->registered_name . ' — Supplier Invoices')
...
@push('styles')
    ...
    <style>
        ...
        .inv-badge-pending { background: #dbeafe; color: #1d4ed8; }
        .inv-badge-approved { background: #dcfce7; color: #15803d; }
        .inv-badge-partially_paid { background: #fef3c7; color: #b45309; }
        .inv-badge-paid { background: #dcfce7; color: #15803d; }
        .inv-badge-disputed { background: #fee2e2; color: #b91c1c; }
        .inv-badge-voided { background: #f3f4f6; color: #6b5b8a; }
        ...
        .status-filter a {
            font-size:6pt; padding:2pt 6pt; border:1px solid #c4b5fd; color:#6b5b8a;
            text-decoration:none; font-weight:600; text-transform:uppercase; letter-spacing:0.05em;
        }
        .status-filter a:hover { background:#f5f3ff; }
        .status-filter a.active { background:#5e17eb; color:#fff; border-color:#5e17eb; }
    </style>
@endpush
                ...
                <div class="reg-doc">
                    <div class="reg-doc-body">
                        <div class="reg-mgmt-bar">
                            <div>
                                <div class="reg-doc-title">Supplier Invoices</div>
                            ...
                            <div style="display:flex;align-items:center;gap:6pt;">
                                ...
                                <a href="{{ route('companies.supplier-invoices.create', $company) }}" class="reg-btn primary">
                        ...
                        <div class="status-filter">
                            <a href="{{ route('companies.supplier-invoices.index', $company) }}" class="{{ !request('status') ? 'active' : '' }}">All</a>
                            @foreach (\App\Enums\SupplierInvoiceStatus::cases() as $s)
                                <a href="{{ route('companies.supplier-invoices.index', [$company, 'status' => $s->value]) }}" class="{{ request('status') === $s->value ? 'active' : '' }}">{{ $s->label() }}</a>
                            @endforeach
                        </div>
                        ...
                        @if ($supplierInvoices->isEmpty())
                            <div style="padding:20pt 10pt;text-align:center;color:#8b7aad;font-size:7pt;">
                                ...
                                <p style="font-weight:700;color:#6b5b8a;margin:0 0 2pt;">No supplier invoices yet</p>
                                <p style="font-size:6.5pt;margin:0 0 6pt;">Create your first supplier invoice to get started.</p>
                                <a href="{{ route('companies.supplier-invoices.create', $company) }}" class="reg-btn primary">
                        ...
                        @else
                                ...
                                <table class="reg-table">
                                    ...
                                    <tbody>
                                        @foreach ($supplierInvoices as $invoice)
                                            <tr class="inv-row" data-search="{{ strtolower($invoice->invoice_number . ' ' . (optional($invoice->supplier)->name ?? '') . ' ' . $invoice->status->label()) }}">
                                                ...
                                                <td>
                                                    <span class="inv-badge inv-badge-{{ $invoice->status->value }}">
                                                ...
                                                <td>
                                                    <div class="reg-row-actions">
                                                        ...
                                                        <div class="reg-row-menu">
                                                            <a href="{{ route('companies.supplier-invoices.show', [$company, $invoice]) }}">View</a>
                                                            <a href="{{ route('companies.supplier-invoices.pdf', [$company, $invoice]) }}" target="_blank">PDF</a>
                                                        </div>
                                                    </div>
                                                </td>
                                            </tr>
                            ...
                            <div style="margin-top:8pt;">
                                {{ $supplierInvoices->withQueryString()->links() }}
                            </div>
                        @endif
                    </div>
                </div>
