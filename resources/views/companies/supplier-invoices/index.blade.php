...
@push('styles')
    ...
    <style>
        ...
        .inv-badge-voided { background: #f4fafc; color: #5a7186; }
                    ...
                    <div class="reg-doc-body">
                        ...
                        @if ($supplierInvoices->isEmpty())
                            <div style="padding:20pt 10pt;text-align:center;color:#6f869b;font-size:7pt;">
                                ...
                                <p style="font-weight:700;color:#5a7186;margin:0 0 2pt;">No supplier invoices yet</p>
                                    ...
                                    <tbody>
                                        @foreach ($supplierInvoices as $invoice)
                                            <tr class="inv-row" data-search="{{ strtolower($invoice->invoice_number . ' ' . (optional($invoice->supplier)->name ?? '') . ' ' . $invoice->status->label()) }}">
                                                ...
                                                <td data-sort="{{ strtolower(optional($invoice->supplier)->name ?? '') }}">{{ optional($invoice->supplier)->name ?? '—' }}</td>
                                                ...
                                                <td data-sort="{{ $invoice->status->value }}">
                                                    <span class="inv-badge inv-badge-{{ $invoice->status->value }}">
                                                        {{ $invoice->status->label() }}
                                                    </span>
                                                </td>
                                                <td class="amt" data-sort="{{ (float) $invoice->total }}">
