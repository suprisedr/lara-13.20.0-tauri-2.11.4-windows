...
@section('title', $governmentGrant->name . ' — Government Grant')
...
@push('styles')
    ...
    <style>
        ...
        .as-field input,.as-field select,.as-field textarea { width:100%;border:1px solid #9ec1f5;padding:3pt 5pt;font-size:7pt;font-family:inherit;color:#1a345b;background:#fff;outline:none;box-sizing:border-box;height:16pt; }
        .as-field textarea { height:auto; }
...
@section('content')
    <div class="co-wrap">
        ...
        <div class="co-body">
            ...
            <main class="co-main">
                ...
                @php $isActive = $governmentGrant->status === 'active'; @endphp

                <div class="inv-mgmt-bar">
                    <div style="display:flex;align-items:center;gap:4pt;flex-wrap:wrap;">
                        @if ($isActive)
                            <button type="button" class="mgmt-btn" onclick="togglePanel('recognise')">Recognise</button>
                            <button type="button" class="mgmt-btn" onclick="togglePanel('amortise')">Amortise</button>
                            <button type="button" class="mgmt-btn danger" onclick="togglePanel('refund')">Refund</button>
                ...
                <div class="cust-doc">
                    <div class="cust-doc-body">
                        <table class="cust-header-table">
                            <tr>
                                <td style="vertical-align:top;width:55%;">
                                    <div style="font-size:11pt;font-weight:800;">{{ $governmentGrant->name }}</div>
                                    <div style="font-size:7pt;color:#6f869b;margin-top:0.2rem;">{{ ucfirst($governmentGrant->grant_type) }} grant &middot; {{ ucfirst($governmentGrant->recognition_method) }} recognition</div>
                                </td>
                                <td style="vertical-align:top;width:45%;">
                                    <div class="doc-title">GOVERNMENT GRANT</div>
                                    <div class="doc-meta-line">Ref: <strong>{{ $governmentGrant->grant_reference ?? '—' }}</strong></div>
                                    <div class="doc-meta-line">Authority: <strong>{{ $governmentGrant->granting_authority ?? '—' }}</strong></div>
                                    <div style="text-align:right;">
                                        <span class="status-box {{ $governmentGrant->status !== 'active' ? 'closed' : '' }}">
                                            {{ ucfirst($governmentGrant->status) }}
                                        </span>
                                    </div>
                                </td>
                            </tr>
                        ...
                        <table style="width:100%;border-collapse:collapse;font-size:7pt;margin-bottom:4pt;">
                            <tr>
                                <td style="width:50%;">
                                    <div><span style="font-weight:700;display:inline-block;width:160px;">Grant date</span>{{ $governmentGrant->grant_date->format('d M Y') }}</div>
                                    @if ($governmentGrant->fulfilment_date)
                                        <div><span style="font-weight:700;display:inline-block;width:160px;">Fulfilment date</span>{{ $governmentGrant->fulfilment_date->format('d M Y') }}</div>
                                    @endif
                                </td>
                                <td style="width:50%;text-align:right;">
                                    @if ($governmentGrant->notes)
                                        <div style="color:#6f869b;font-style:italic;font-size:6.5pt;">{{ $governmentGrant->notes }}</div>
                                    @endif
                                </td>
                            </tr>
                        </table>

                        @if ($governmentGrant->conditions_text)
                            <div style="background:#f4fafc;border:1px solid #d3e2f5;padding:4pt 6pt;font-size:6.5pt;color:#5a7186;margin-bottom:6pt;line-height:1.4;">
                                <span style="font-weight:700;text-transform:uppercase;font-size:5pt;letter-spacing:0.06em;">Conditions:</span><br>
                                {{ $governmentGrant->conditions_text }}
                            </div>
                        @endif
                        ...
                        <table class="summary-table">
                            <tr>
                                <td>
                                    ...
                                    <span class="amt" style="color:#005bf0;">R {{ number_format((float) ($governmentGrant->total_amount ?? 0), 2) }}</span>
                                </td>
                                <td>
                                    <span class="lbl">Recognised Amount</span>
                                    <span class="amt">R {{ number_format((float) ($governmentGrant->recognised_amount ?? 0), 2) }}</span>
                                </td>
                                <td>
                                    <span class="lbl">Deferred Amount</span>
                                    <span class="amt">R {{ number_format((float) ($governmentGrant->deferred_amount ?? 0), 2) }}</span>
                                </td>
                            </tr>
                        </table>
                        ...
                        @if ($governmentGrant->status === 'active')
                        <div class="action-section">
                            <div class="section-header">IAS 20 Actions</div>

                            <div class="action-panel" id="panel-recognise">
                                <div class="action-panel-head" onclick="togglePanel('recognise')">
                                    <span class="action-panel-title">Recognise grant <small style="font-weight:400;color:#6f869b;">(IAS 20.7)</small></span>
                                    <span class="action-panel-chevron">&#9660;</span>
                                </div>
                                <div class="action-panel-body">
                                    <p class="af-hint">A government grant shall be recognised when there is reasonable assurance that the entity will comply with the conditions and the grant will be received.</p>
                                    <form method="POST" action="{{ route('companies.government-grants.recognise', [$company, $governmentGrant]) }}"
                                        onsubmit="return false" data-confirm-label="Government Grants" data-ajax-confirm data-confirm-title="Recognise" data-confirm-body="Recognise government grant?" data-confirm-text="Save">
                                        @csrf
                                        <div class="af-row">
                                            <div class="af-field">
                            ...
                            <div class="action-panel" id="panel-amortise">
                                <div class="action-panel-head" onclick="togglePanel('amortise')">
                                    <span class="action-panel-title">Amortise to income <small style="font-weight:400;color:#6f869b;">(IAS 20.12)</small></span>
                                    <span class="action-panel-chevron">&#9660;</span>
                                </div>
                                <div class="action-panel-body">
                                    <p class="af-hint">Government grants shall be recognised in profit or loss on a systematic basis over the periods in which the entity recognises as expenses the related costs.</p>
                                    <form method="POST" action="{{ route('companies.government-grants.amortise', [$company, $governmentGrant]) }}"
                                        onsubmit="return false" data-confirm-label="Government Grants" data-ajax-confirm data-confirm-title="Amortise" data-confirm-body="Amortise grant to income?" data-confirm-text="Save">
                                        @csrf
                                        <div class="af-row">
                                            <div class="af-field">
                                                <label>Amount (R)</label>
                            ...
                            <div class="action-panel" id="panel-refund">
                                <div class="action-panel-head" onclick="togglePanel('refund')">
                                    <span class="action-panel-title">Refund <small style="font-weight:400;color:#6f869b;">(IAS 20.32)</small></span>
                                    <span class="action-panel-chevron">&#9660;</span>
                                </div>
                                <div class="action-panel-body">
                                    <p class="af-hint" style="color:#dc2626;">A government grant that becomes repayable shall be accounted for as a change in accounting estimate.</p>
                                    <form method="POST" action="{{ route('companies.government-grants.refund', [$company, $governmentGrant]) }}"
                                        onsubmit="return false" data-confirm-label="Government Grants" data-ajax-confirm data-confirm-title="Refund" data-confirm-body="Record grant refund?" data-confirm-text="Refund" data-confirm-danger="1">
                                        @csrf
                                        <div class="af-row">
                                            <div class="af-field">
                                                <label>Amount (R)</label>
                                        ...
                                        <div style="display:flex;justify-content:flex-end;">
                                            <button type="submit" class="mgmt-btn danger">Refund</button>
                                        </div>
                                    </form>
                                </div>
                            </div>
    ...
    <script>
        ...
        (function() {
            ...
            fetch('{{ route("companies.government-grants.history", [$company, $governmentGrant]) }}')
                .then(r => r.json())
                .then(events => {
    ...
    <div id="edit-modal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,0.55);z-index:9999;align-items:center;justify-content:center;padding:16pt 8pt;">
        <div style="background:#fff;width:100%;max-width:420px;border:1px solid #1a345b;box-shadow:0 20px 60px rgba(0,0,0,0.4);margin:0 auto;">
            ...
            <form method="POST" action="{{ route('companies.government-grants.update', [$company, $governmentGrant]) }}">
                @csrf @method('PATCH')
                <div style="padding:10pt 12pt;">
                    <div style="display:grid;grid-template-columns:1fr 1fr;gap:5pt 7pt;margin-bottom:8pt;">
                        <div class="as-field" style="grid-column:1/-1;">
                            <label>Name</label>
                            <input type="text" name="name" required value="{{ $governmentGrant->name }}">
                        </div>
                        <div class="as-field" style="grid-column:1/-1;">
                            <label>Granting authority</label>
                            <input type="text" name="granting_authority" value="{{ $governmentGrant->granting_authority }}">
                        </div>
                        <div class="as-field" style="grid-column:1/-1;">
                            <label>Conditions</label>
                            <textarea name="conditions_text" rows="3" style="height:auto;">{{ $governmentGrant->conditions_text }}</textarea>
                        </div>
                        <div class="as-field" style="grid-column:1/-1;">
                            <label>Notes</label>
                            <input type="text" name="notes" value="{{ $governmentGrant->notes }}" maxlength="500">
                        </div>
                    </div>
