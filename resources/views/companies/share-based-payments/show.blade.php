...
@section('title', $arrangement->name . ' — Share-Based Payment')
...
@section('content')
    <div class="co-wrap">
        ...
        <div class="co-body">
            ...
            <main class="co-main">
                ...
                @php $isActive = $arrangement->status === 'active'; @endphp

                <div class="inv-mgmt-bar">
                    <div style="display:flex;align-items:center;gap:4pt;flex-wrap:wrap;">
                        @if ($isActive)
                            <button type="button" class="mgmt-btn" onclick="togglePanel('vesting-expense')">Record Expense</button>
                            <button type="button" class="mgmt-btn" onclick="togglePanel('exercise')">Exercise</button>
                            <button type="button" class="mgmt-btn danger" onclick="togglePanel('forfeit')">Forfeit</button>
                ...
                <div class="cust-doc">
                    <div class="cust-doc-body">
                        <table class="cust-header-table">
                            <tr>
                                <td style="vertical-align:top;width:55%;">
                                    <div style="font-size:11pt;font-weight:800;">{{ $arrangement->name }}</div>
                                    <div style="font-size:7pt;color:#6f869b;margin-top:0.2rem;">{{ ucfirst(str_replace('_', ' ', $arrangement->arrangement_type)) }}</div>
                                </td>
                                <td style="vertical-align:top;width:45%;">
                                    <div class="doc-title">SHARE-BASED PAYMENT</div>
                                    <div class="doc-meta-line">Grant date: <strong>{{ $arrangement->grant_date->format('d M Y') }}</strong></div>
                                    @if ($arrangement->vesting_start_date)
                                        <div class="doc-meta-line">Vesting start: <strong>{{ $arrangement->vesting_start_date->format('d M Y') }}</strong></div>
                                    @endif
                                    @if ($arrangement->vesting_end_date)
                                        <div class="doc-meta-line">Vesting end: <strong>{{ $arrangement->vesting_end_date->format('d M Y') }}</strong></div>
                                    @endif
                                    @if ($arrangement->exercise_price)
                                        <div class="doc-meta-line">Exercise price: <strong>R {{ number_format((float) $arrangement->exercise_price, 2) }}</strong></div>
                                    @endif
                                    <div style="text-align:right;">
                                        <span class="status-box {{ !$isActive ? 'closed' : '' }}">
                        ...
                        <table style="width:100%;border-collapse:collapse;font-size:7pt;margin-bottom:4pt;">
                            <tr>
                                <td style="width:50%;">
                                    @if ($arrangement->vesting_conditions)
                                        <div><span style="font-weight:700;display:inline-block;width:160px;">Vesting conditions</span>{{ $arrangement->vesting_conditions }}</div>
                                    @endif
                                </td>
                                <td style="width:50%;text-align:right;">
                                    @if ($arrangement->notes)
                                        <div style="color:#6f869b;font-style:italic;font-size:6.5pt;">{{ $arrangement->notes }}</div>
                                    @endif
                                </td>
                            </tr>
                        </table>
                        ...
                        <table class="summary-table">
                            <tr>
                                <td>
                                    <span class="lbl">FV at Grant</span>
                                    <span class="amt" style="color:#005bf0;">R {{ number_format((float) $arrangement->fair_value_at_grant, 2) }}</span>
                                </td>
                                <td>
                                    <span class="lbl">Number of Instruments</span>
                                    <span class="amt">{{ number_format($arrangement->number_of_instruments) }}</span>
                                </td>
                                <td>
                                    <span class="lbl">Total Expense</span>
                                    <span class="amt">R {{ number_format((float) ($arrangement->total_expense ?? 0), 2) }}</span>
                                </td>
                                <td>
                                    <span class="lbl">Vesting %</span>
                                    <span class="amt">{{ number_format($arrangement->vestingPercentage(), 1) }}%</span>
                                </td>
                            </tr>
                        </table>
                        ...
                        <div class="action-section">
                            <div class="section-header">IFRS 2 Actions</div>

                            <div class="action-panel" id="panel-vesting-expense">
                                <div class="action-panel-head" onclick="togglePanel('vesting-expense')">
                                    <span class="action-panel-title">Record vesting expense <small style="font-weight:400;color:#6f869b;">(IFRS 2.7)</small></span>
                                    <span class="action-panel-chevron">&#9660;</span>
                                </div>
                                <div class="action-panel-body">
                                    <p class="af-hint">The entity shall recognise the services received, and a corresponding increase in equity or liability, as the employees render service.</p>
                                    <form method="POST" action="{{ route('companies.share-based-payments.vesting-expense', [$company, $arrangement]) }}"
                                        onsubmit="return false" data-confirm-label="Share-Based Payments" data-ajax-confirm data-confirm-title="Record Expense" data-confirm-body="Record vesting expense?" data-confirm-text="Save">
                                        @csrf
                                        <div class="af-row">
                                            <div class="af-field">
                            ...
                            <div class="action-panel" id="panel-exercise">
                                <div class="action-panel-head" onclick="togglePanel('exercise')">
                                    <span class="action-panel-title">Exercise instruments <small style="font-weight:400;color:#6f869b;">(IFRS 2.23)</small></span>
                                    <span class="action-panel-chevron">&#9660;</span>
                                </div>
                                <div class="action-panel-body">
                                    <p class="af-hint">When equity instruments are exercised, the amounts accumulated in the equity component are transferred within equity.</p>
                                    <form method="POST" action="{{ route('companies.share-based-payments.exercise', [$company, $arrangement]) }}"
                                        onsubmit="return false" data-confirm-label="Share-Based Payments" data-ajax-confirm data-confirm-title="Exercise" data-confirm-body="Exercise instruments?" data-confirm-text="Save">
                                        @csrf
                                        <div class="af-row">
                                            <div class="af-field">
                                                <label>Instruments to exercise</label>
                                                <input type="number" name="instruments" min="1" required>
                                            </div>
                                            <div class="af-field">
                                                <label>Date</label>
                                                <input type="date" name="date" required value="{{ now()->format('Y-m-d') }}">
                            ...
                            <div class="action-panel" id="panel-forfeit">
                                <div class="action-panel-head" onclick="togglePanel('forfeit')">
                                    <span class="action-panel-title">Forfeit instruments <small style="font-weight:400;color:#6f869b;">(IFRS 2.28)</small></span>
                                    <span class="action-panel-chevron">&#9660;</span>
                                </div>
                                <div class="action-panel-body">
                                    <p class="af-hint">If vested equity instruments are forfeited, the entity shall reverse the cumulative expense recognised for those instruments.</p>
                                    <form method="POST" action="{{ route('companies.share-based-payments.forfeit', [$company, $arrangement]) }}"
                                        onsubmit="return false" data-confirm-label="Share-Based Payments" data-ajax-confirm data-confirm-title="Forfeit" data-confirm-body="Forfeit instruments? This will reverse the cumulative expense." data-confirm-text="Forfeit" data-confirm-danger="1">
                                        @csrf
                                        <div class="af-row">
                                            <div class="af-field">
                                                <label>Instruments to forfeit</label>
                                                <input type="number" name="instruments" min="1" required>
                                            </div>
                                            <div class="af-field">
                                                <label>Date</label>
                                        ...
                                        <div style="display:flex;justify-content:flex-end;">
                                            <button type="submit" class="mgmt-btn danger">Forfeit</button>
                                        </div>
                                    </form>
                                </div>
                            </div>
    ...
    <script>
        ...
        (function() {
            ...
            fetch('{{ route("companies.share-based-payments.history", [$company, $arrangement]) }}')
                .then(r => r.json())
                .then(events => {
                    ...
                    events.forEach(function(e) {
                        ...
                        var instrText = e.instruments ? ' — ' + e.instruments + ' instrument' + (e.instruments !== 1 ? 's' : '') : '';
                        ...
                        div.innerHTML =
                            ...
                            '<span class="history-amt">R ' + e.amount + instrText + '</span>' +
                            (e.transaction_url
    ...
    <div id="edit-modal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,0.55);z-index:9999;align-items:center;justify-content:center;padding:16pt 8pt;">
        <div style="background:#fff;width:100%;max-width:420px;border:1px solid #1a345b;box-shadow:0 20px 60px rgba(0,0,0,0.4);margin:0 auto;">
            ...
            <form method="POST" action="{{ route('companies.share-based-payments.update', [$company, $arrangement]) }}">
                @csrf @method('PATCH')
                <div style="padding:10pt 12pt;">
                    <div style="display:grid;grid-template-columns:1fr 1fr;gap:5pt 7pt;margin-bottom:8pt;">
                        <div class="as-field" style="grid-column:1/-1;">
                            <label>Name</label>
                            <input type="text" name="name" required value="{{ $arrangement->name }}">
                        </div>
                        <div class="as-field">
                            <label>Vesting start date</label>
                            <input type="date" name="vesting_start_date" value="{{ $arrangement->vesting_start_date?->format('Y-m-d') }}">
                        </div>
                        <div class="as-field">
                            <label>Vesting end date</label>
                            <input type="date" name="vesting_end_date" value="{{ $arrangement->vesting_end_date?->format('Y-m-d') }}">
                        </div>
                        <div class="as-field" style="grid-column:1/-1;">
                            <label>Vesting conditions</label>
                            <input type="text" name="vesting_conditions" value="{{ $arrangement->vesting_conditions }}" maxlength="500">
                        </div>
                        <div class="as-field" style="grid-column:1/-1;">
                            <label>Notes</label>
                            <input type="text" name="notes" value="{{ $arrangement->notes }}" maxlength="500">
                        </div>
                    </div>
