...
<head>
    ...
    <title>{{ $certificate['certificate_type'] }} - {{ $certificate['employee']->full_name }}</title>
    <style>
        @page {
            margin: 0;
            size: A4 landscape;
...
<body>
    ...
    <div class="cert-header">
        ...
        <p style="font-size:7pt;color:#5a7186;">Period: {{ \Carbon\Carbon::parse($certificate['tax_year_start'])->format('d/m/Y') }} to {{ \Carbon\Carbon::parse($certificate['tax_year_end'])->format('d/m/Y') }}</p>
    </div>

    {{-- Employer and Employee details side by side --}}
    <table class="two-col">
        <tr>
            <td>
                ...
                <table class="info-table">
                    ...
                    @if ($company->address_line_1)
                        <tr><td class="lbl">Address</td><td class="val">{{ implode(', ', array_filter([$company->address_line_1, $company->city, $company->province, $company->postal_code])) }}</td></tr>
                    @endif
                </table>
            </td>
            <td>
                ...
                <table class="info-table">
                    ...
                    @if ($emp->passport_number)
                        <tr><td class="lbl">Passport No.</td><td class="val">{{ $emp->passport_number }}</td></tr>
                    @endif
                    ...
                    <tr><td class="lbl">Employment Type</td><td class="val">{{ ucfirst(str_replace('_', ' ', $emp->employment_type ?? '---')) }}</td></tr>
                    <tr><td class="lbl">Periods Employed</td><td class="val">{{ $certificate['periods_employed'] }} month(s)</td></tr>
                    @if ($emp->address_line_1)
                        <tr><td class="lbl">Address</td><td class="val">{{ implode(', ', array_filter([$emp->address_line_1, $emp->city, $emp->province, $emp->postal_code])) }}</td></tr>
                    @endif
                </table>
            </td>
        </tr>
    ...
    {{-- Income and deductions side by side --}}
    <table class="two-col">
        <tr>
            <td>
    ...
    <div class="footer-text">
        This certificate is issued by {{ $company->registered_name }} for the {{ $certificate['tax_year_label'] }} tax year.
        Generated from payroll records ({{ $certificate['payslip_count'] }} payslip(s)). Verify against SARS requirements before submission.
    </div>
