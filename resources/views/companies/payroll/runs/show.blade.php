...
@push('styles')
    ...
    <style>
        ...
        .run-chip.highlight { border-color: #4c1d95; }
                    ...
                    <div class="reg-doc-body">
                        ...
                        <div class="afs-letterhead">
                            ...
                            <div class="afs-letterhead-meta afs-right">
                                <div style="font-size:9pt;font-weight:800;color:#4c1d95;margin-bottom:3pt;">{{ $run->period_label }}</div>
                                @if ($run->notes)
                                    <div style="font-size:6.5pt;color:#6b5b8a;font-style:italic;">{{ $run->notes }}</div>
                                @endif
                            </div>
                        </div>
                        ...
                        <div class="reg-section-header" style="margin-top:8pt;">
                            Run Summary
                            <span style="float:right;font-size:5.5pt;font-weight:400;text-transform:none;letter-spacing:0;color:#8b7aad;">{{ $payslips->count() }} employee{{ $payslips->count() !== 1 ? 's' : '' }}</span>
                        </div>
                        <div class="run-chips">
                            <div class="run-chip">
                        ...
                        @if ($debitLines->isNotEmpty())
                            <div style="overflow-x:auto;">
                                <table class="reg-table">
                                    ...
                                    <tfoot>
                                        <tr>
                                            <td style="color:#8b7aad;">Total payroll cost</td>
                        ...
                        @else
                            <p style="font-size:7pt;color:#8b7aad;">No payslips yet — calculate the run first.</p>
                        @endif
                        ...
                        @if ($run->employee_type === 'hourly' && ! $run->isPosted())
                            <div class="reg-section-header" style="margin-top:8pt;">
                                Hours Worksheet
                                <span style="float:right;font-size:5.5pt;font-weight:400;text-transform:none;letter-spacing:0;color:#8b7aad;">Enter hours &rarr; save &rarr; recalculate</span>
                            </div>
                            <form method="POST" action="{{ route('companies.payroll.runs.hours-worksheet', [$company, $run]) }}">
                                            ...
                                            @foreach ($payslips as $i => $payslip)
                                                ...
                                                <tr>
                                                    <td>
                                                        ...
                                                        <span style="font-size:6pt;color:#8b7aad;font-family:'DejaVu Sans Mono',monospace;">{{ $payslip->employee->employee_number }}</span>
                                                    </td>
                                                    <td style="color:#6b5b8a;">R {{ number_format($payslip->employee->hourly_rate, 2) }}/hr</td>
                                                    ...
                                                    <td>
                                                        <input type="number"
                                                            ...
                                                            style="width:100%;border:0.5pt solid #c4b5fd;padding:3pt 5pt;font-size:7pt;font-family:inherit;outline:none;box-sizing:border-box;color:#23282d;"
                                                            onfocus="this.style.borderColor='#4c1d95'" onblur="this.style.borderColor='#c4b5fd'">
                                                    </td>
                                                </tr>
                                            @endforeach
                                        </tbody>
                                ...
                                <div style="margin-top:6pt;display:flex;gap:6pt;align-items:center;">
                                    ...
                                    <span style="font-size:6pt;color:#8b7aad;">Supports quarter-hour increments (0.25).</span>
                                </div>
                            </form>
                        @endif
                        ...
                        <div class="reg-section-header" style="margin-top:8pt;">
                            Individual Payslips
                            <span style="float:right;font-size:5.5pt;font-weight:400;text-transform:none;letter-spacing:0;color:#8b7aad;">{{ $payslips->count() }} payslip{{ $payslips->count() !== 1 ? 's' : '' }}</span>
                        </div>
                        <div style="overflow-x:auto;">
                            <table class="reg-table">
                                <thead>
                        ...
                        <div class="reg-section-header" style="margin-top:8pt;">
                            Payslip Detail
                            <span style="float:right;font-size:5.5pt;font-weight:400;text-transform:none;letter-spacing:0;color:#8b7aad;">Click a row to expand</span>
                        </div>

                        @foreach ($payslips as $payslip)
                            <div style="border:0.5pt solid #c4b5fd;margin-bottom:4pt;" x-data="{ open: false }">
                                <div style="padding:5pt 7pt;display:flex;justify-content:space-between;align-items:center;cursor:pointer;user-select:none;background:#f5f3ff;"
                                    @click="open = !open">
                                    <div>
                                        <span style="font-weight:700;font-size:7pt;">{{ $payslip->employee->full_name }}</span>
                                        <span style="font-size:6pt;color:#8b7aad;font-family:'DejaVu Sans Mono',monospace;margin-left:4pt;">{{ $payslip->employee->employee_number }}</span>
                                        @if ($payslip->is_adjusted)
                                    ...
                                    <div style="display:flex;align-items:center;gap:8pt;">
                                        <span style="font-size:6.5pt;color:#8b7aad;">Net: <strong style="color:#15803d;">R {{ number_format($payslip->net_pay, 2) }}</strong></span>
                                        <svg width="10" height="10" fill="none" stroke="#8b7aad" stroke-width="2" viewBox="0 0 24 24"
                                ...
                                <div x-show="open" x-cloak style="border-top:0.5pt solid #c4b5fd;">
                                    <div style="display:grid;grid-template-columns:1fr 1fr;gap:8pt;padding:8pt 7pt;">
                                        ...
                                        <div>
                                            ...
                                            @endforeach
                                            <div class="detail-row" style="font-weight:700;border-top:0.5pt solid #4c1d95;margin-top:2pt;padding-top:2pt;">
                                    ...
                                    @if (! $run->isPosted())
                                        <div style="border-top:0.5pt solid #c4b5fd;padding:6pt 7pt;background:#faf5ff;">
                                            ...
                                            <form method="POST" action="{{ route('companies.payroll.payslip.adjust', [$company, $run, $payslip]) }}">
                                                ...
                                                <p style="font-size:5.5pt;color:#8b7aad;margin:3pt 0 0;">PAYE, UIF and SDL recalculate automatically on save.</p>
                                            </form>
                                        </div>
                                    @endif

                                    <div style="padding:4pt 7pt;border-top:0.5pt solid #c4b5fd;text-align:right;">
