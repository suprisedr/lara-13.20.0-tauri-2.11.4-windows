...
@push('styles')
    ...
    <style>
        .dn-form { max-width:860px; }
        .dn-section { margin-bottom:1.75rem; }
        .dn-section-title {
            font-size:0.65rem; font-weight:800; letter-spacing:0.1em;
            text-transform:uppercase; color:#5e17eb; margin:0 0 0.75rem;
        }
        .dn-grid { display:grid; grid-template-columns:1fr 1fr; gap:0.75rem 1.25rem; }
        .dn-grid.wide { grid-template-columns:1fr; }
        ...
        .dn-field input:focus, .dn-field select:focus, .dn-field textarea:focus {
            outline:none; border-color:#5e17eb;
        }
        ...
        .inv-hit {
            padding:0.5rem 0.8rem; cursor:pointer; font-size:0.8rem;
            border-bottom:1px solid #f3f0ff;
        }
        .inv-hit:hover { background:#faf5ff; }
        .inv-hit-number { font-weight:700; color:#1b1b18; }
        .inv-hit-sub { font-size:0.7rem; color:#6b7280; margin-top:1px; }
        .dn-field textarea { resize:vertical; min-height:70px; }
        .dn-field.span2 { grid-column:span 2; }
        .dn-field.span3 { grid-column:span 3; }

        /* Line items */
        .items-table { width:100%; border-collapse:collapse; margin-bottom:0.75rem; }
        .items-table th {
            font-size:0.6rem; font-weight:700; letter-spacing:0.06em; text-transform:uppercase;
            color:#555; padding:0 0 0.4rem; border-bottom:1.5px solid #000; text-align:left;
        }
        .items-table td { padding:0.4rem 0; border-bottom:1px solid #e5e7eb; vertical-align:top; }
        .items-table input, .items-table select {
            width:100%; border:1px solid #ccc; padding:0.28rem 0.45rem;
            font-size:0.78rem; font-family:inherit; background:#fff; box-sizing:border-box;
        }
        .items-table input:focus, .items-table select:focus { outline:none; border-color:#5e17eb; }
        .btn-remove-row {
            background:none; border:none; cursor:pointer; color:#dc2626; font-size:1rem;
            padding:0.1rem 0.3rem; line-height:1;
        }
        .btn-add-row {
            background:none; border:1px dashed #ccc; padding:0.4rem 0.8rem;
            font-size:0.72rem; font-weight:600; cursor:pointer; color:#555;
            transition:border-color 0.15s; margin-top:0.25rem;
        }
        .btn-add-row:hover { border-color:#5e17eb; color:#5e17eb; }
        .btn-submit {
            background:#000; color:#fff; border:none; padding:0.55rem 1.5rem;
    ...
    <div class="co-wrap">
        ...
        <div class="co-body">
            ...
            <main class="co-main">
                ...
                <form method="POST" action="{{ route('companies.delivery-notes.store', $company) }}" class="dn-form" id="dn-form">
                    @csrf
                    ...
                    <div class="dn-section">
                        <p class="dn-section-title">Link to Invoice (optional)</p>
                        <div class="dn-grid">
                            <div class="dn-field">
                                <label>Invoice</label>
                                ...
                                <div class="inv-search-wrap">
                                    <input type="text" id="invoice-search" class="inv-search-input" autocomplete="off"
                    ...
                    {{-- Header details --}}
                    <div class="dn-section">
                        <p class="dn-section-title">Delivery Details</p>
                        <div class="dn-grid three">
                            <div class="dn-field">
                        ...
                        <div class="dn-grid three" style="margin-top:0.75rem;">
                            <div class="dn-field">
                                <label>Status</label>
                                <select name="status">
                        ...
                        <div class="dn-grid" style="margin-top:0.75rem;">
                            <div class="dn-field">
                    ...
                    {{-- Customer --}}
                    <div class="dn-section">
                        <p class="dn-section-title">Deliver To</p>
                        <div class="dn-grid">
                            <div class="dn-field">
                            ...
                            <div class="dn-field">
                                ...
                                <textarea name="delivery_address" id="delivery-address" rows="3">{{ old('delivery_address', $invoice?->customer_address) }}</textarea>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="dn-section">
                        ...
                        <button type="button" class="btn-add-row" id="add-row-btn" onclick="addRow()">+ Add Item</button>
                        @error('items')<p class="form-error">{{ $message }}</p>@enderror
                    </div>

                    {{-- Notes --}}
                    <div class="dn-section">
                        <p class="dn-section-title">Notes</p>
                        <textarea name="notes" rows="3" style="width:100%;border:1px solid #ccc;padding:0.4rem 0.6rem;font-size:0.8rem;font-family:inherit;">{{ old('notes') }}</textarea>
                    </div>
                    ...
                    <div style="display:flex;gap:1rem;align-items:center;">
                        ...
                        <a href="{{ route('companies.delivery-notes.index', $company) }}"
                            style="font-size:0.78rem;color:#6b7280;text-decoration:none;">Cancel</a>
                    </div>
    ...
    <script>
        ...
        (function () {
            ...
            function selectInvoice(inv) {
                ...
                // Repopulate items table from invoice lines
            ...
            // On page load: if pre-linked invoice, show badge
            if (hiddenEl.value) {
