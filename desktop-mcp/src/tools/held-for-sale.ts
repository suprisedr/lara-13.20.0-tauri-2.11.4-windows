...
export const tools: Tool[] = [
  {
    ...
    description:
      "Fetch IFRS 5 assets held for sale for a company. " +
  ...
  },
  {
    ...
    description:
      "Fetch a single IFRS 5 held-for-sale asset by ID with full details. Requires prior login.",
    inputSchema: {
      type: "object",
      properties: {
  ...
  },
  {
    ...
    description:
      "IFRS 5 Reclassification — reclassify an active PPE asset as held for sale. " +
  ...
  },
  {
    ...
    description:
      "IFRS 5.26-29 Reversal — reverse the held-for-sale classification and return the asset to PPE. " +
  ...
  },
  {
    ...
    description:
      "IFRS 5.25 Disposal — record the actual sale of a held-for-sale asset. " +
...
async function handleReclassify(args: Record<string, unknown>): Promise<ToolResult> {
  const { asset_id, ...body } = args;
  const result = await apiPost(`/assets/${asset_id}/reclassify-held-for-sale`, body as Record<string, unknown>);
  return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
}

async function handleReverse(args: Record<string, unknown>): Promise<ToolResult> {
  const { id, ...body } = args;
  const result = await apiPost(`/held-for-sale/${id}/reverse`, body as Record<string, unknown>);
  return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
}

async function handleSell(args: Record<string, unknown>): Promise<ToolResult> {
  const { id, ...body } = args;
  const result = await apiPost(`/held-for-sale/${id}/dispose`, body as Record<string, unknown>);
  return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
}
