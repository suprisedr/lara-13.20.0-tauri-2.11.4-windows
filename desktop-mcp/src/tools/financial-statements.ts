...
import { AfsModel, buildAfsDocument } from "../lib/afs-docx.js";

type ToolResult = { content: Array<{ type: "text"; text: string }>; isError?: boolean };

/** Every statement the tool can render, in presentation order. */
const STATEMENT_KEYS = ["income-statement", "balance-sheet", "cash-flow", "changes-in-equity"] as const;
...
export const tools: Tool[] = [
  {
    ...
    description:
      "Generate the annual financial statements as a Word document (.docx) and save it to the user's " +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        statements: {
          ...
          description:
            "Which statements to include. Defaults to all four. Order in the document is always " +
        ...
        filename: {
          type: "string",
          description:
            "File name without extension. Defaults to '<company>-annual-financial-statements-<year>'. " +
...
export async function handle(name: string, args: Record<string, unknown>): Promise<ToolResult | null> {
  if (name !== "generate_financial_statements") return null;
  ...
  const requested = Array.isArray(args.statements) ? (args.statements as string[]) : [...STATEMENT_KEYS];
  const unknown = requested.filter((k) => !STATEMENT_KEYS.includes(k as (typeof STATEMENT_KEYS)[number]));
  if (unknown.length > 0) {
  ...
  const slug = (args.filename as string | undefined) ?? `${slugify(model.company.name)}-annual-financial-statements-${year}`;
