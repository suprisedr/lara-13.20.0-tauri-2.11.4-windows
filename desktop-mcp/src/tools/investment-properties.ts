...
export const tools: Tool[] = [
  {
    ...
    description:
      "Fetch IAS 40 investment properties for a company. " +
  ...
  },
  {
    ...
    description:
      "Fetch a single IAS 40 investment property by ID with full details. Requires prior login.",
    inputSchema: {
      type: "object",
      properties: {
  ...
  },
  {
    ...
    description:
      "Register a new IAS 40 investment property. " +
  ...
  },
  {
    ...
    description:
      "Update mutable fields on an IAS 40 investment property (name, reference, location, residual_value, useful_life_years, depreciation_method, notes). " +
      "Requires prior login." +
      FLAG_RULE,
    inputSchema: {
      type: "object",
  ...
  },
  {
    ...
    description:
      "IAS 40.35 Fair Value Adjustment — record a fair value change for an investment property measured under the fair value model. " +
  ...
  },
  {
    ...
    description:
      "IAS 36 Impairment — record an impairment event for an investment property measured under the cost model. " +
  ...
  },
  {
    ...
    description:
      "IAS 36.114 Impairment Reversal — reverse a previously recognised impairment on an investment property (cost model). " +
      "Requires prior login." +
      FLAG_RULE,
    inputSchema: {
      type: "object",
  ...
  },
  {
    ...
    description:
      "IAS 40.17 Subsequent Expenditure — capitalise subsequent costs that meet recognition criteria for an investment property. " +
      "Requires prior login." +
      FLAG_RULE,
    inputSchema: {
      type: "object",
  ...
  },
  {
    ...
    description:
      "IAS 40.66 Disposal — dispose of an investment property on sale or retirement. " +
...
async function handleGetProperties(args: Record<string, unknown>): Promise<ToolResult> {
  const data = await apiGet(`/investment-properties?company_id=${args.company_id}`);
  return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
}

async function handleGetProperty(args: Record<string, unknown>): Promise<ToolResult> {
...
async function handleFairValueAdjust(args: Record<string, unknown>): Promise<ToolResult> {
  const { id, ...body } = args;
  const result = await apiPost(`/investment-properties/${id}/fair-value-adjust`, body as Record<string, unknown>);
  return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
}

async function handleImpair(args: Record<string, unknown>): Promise<ToolResult> {
  const { id, ...body } = args;
  const result = await apiPost(`/investment-properties/${id}/impair`, body as Record<string, unknown>);
  return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
}

async function handleReverseImpairment(args: Record<string, unknown>): Promise<ToolResult> {
  const { id, ...body } = args;
  const result = await apiPost(`/investment-properties/${id}/reverse-impairment`, body as Record<string, unknown>);
  return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
}

async function handleCapitalise(args: Record<string, unknown>): Promise<ToolResult> {
  const { id, ...body } = args;
  const result = await apiPost(`/investment-properties/${id}/capitalise`, body as Record<string, unknown>);
  return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
}

async function handleDispose(args: Record<string, unknown>): Promise<ToolResult> {
  const { id, ...body } = args;
  const result = await apiPost(`/investment-properties/${id}/dispose`, body as Record<string, unknown>);
  return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
}
