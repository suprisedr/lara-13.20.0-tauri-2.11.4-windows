...
import { MgmtModel, buildManagementWorkbook } from "../lib/management-xlsx.js";

type ToolResult = { content: Array<{ type: "text"; text: string }>; isError?: boolean };

/** Every sheet the tool can render, in presentation order. */
const SHEET_KEYS = ["trading", "monthly-trend", "financial-position", "cash-flow", "ratios", "trial-balance"] as const;
...
export const tools: Tool[] = [
  {
    ...
    description:
      "Generate a monthly management accounts pack as an Excel workbook (.xlsx) and save it to the user's " +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        period_end: {
          type: "string",
          description:
            "Last day of the reporting period, in YYYY-MM-DD. Defaults to the end of the month just " +
        ...
        ytd_start: {
          type: "string",
          description:
            "Start of the year-to-date span, in YYYY-MM-DD. Defaults to the start of the financial year " +
        ...
        sheets: {
          ...
          description:
            "Which sheets to include. Defaults to all six. Order in the workbook is always cover, " +
        ...
        compare: {
          type: "boolean",
          description:
            "Include prior-period comparatives and the variance columns. Defaults to true. With false the " +
            "pack shows actuals only.",
        },
        filename: {
          type: "string",
          description:
            "File name without extension. Defaults to '<company>-management-accounts-<YYYY-MM>'. " +
...
export async function handle(name: string, args: Record<string, unknown>): Promise<ToolResult | null> {
  if (name !== "generate_management_accounts") return null;
  ...
  const requested = Array.isArray(args.sheets) ? (args.sheets as string[]) : [...SHEET_KEYS];
  const unknown = requested.filter((k) => !SHEET_KEYS.includes(k as (typeof SHEET_KEYS)[number]));
  if (unknown.length > 0) {
  ...
  const model = (await getManagementAccountsModel({
    companyId,
    periodStart: args.period_start as string | undefined,
    periodEnd: args.period_end as string | undefined,
    ytdStart: args.ytd_start as string | undefined,
  ...
  const slug = (args.filename as string | undefined) ?? `${slugify(model.company.name)}-management-accounts-${month}`;
