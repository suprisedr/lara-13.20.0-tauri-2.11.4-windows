...
export const tools: Tool[] = [
  {
    name: "inventory",
    description:
      "Fetch inventory items (IAS 2) and service items for a company. " +
  ...
  },
  {
    ...
    description:
      "Fetch a single inventory item by ID, including its recent stock movements (last 50). " +
  ...
  },
  {
    ...
    description:
      "Create a new inventory item or service item. " +
  ...
  },
  {
    ...
    description:
      "Update an existing inventory item's details (name, description, sku, unit_price, purchase_cost, tax_rate, inventory_type, is_active). " +
  ...
  },
  {
    ...
    description:
      "Record a stock movement (receive, issue, or adjust) for an inventory item. " +
  ...
  },
  {
    ...
    description:
      "IAS 2.9 / IAS 2.34 — Write down inventory to net realisable value (NRV) when NRV falls below cost. " +
  ...
  },
  {
    ...
    description:
      "IAS 2.33 — Reverse a previous NRV write-down when the circumstances that caused the write-down no longer exist " +
...
async function handleList(args: Record<string, unknown>): Promise<ToolResult> {
  ...
  if (args.only_active !== undefined) params.set("only_active", String(args.only_active));
  if (args.include_services !== undefined) params.set("include_services", String(args.include_services));

  const data = await apiGet(`/inventory?${params.toString()}`);
  return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
}

async function handleShow(args: Record<string, unknown>): Promise<ToolResult> {
...
async function handleMovement(args: Record<string, unknown>): Promise<ToolResult> {
  const { id, ...fields } = args;
  const body: Record<string, unknown> = {
  ...
  if (fields.unit_cost !== undefined) body.unit_cost = fields.unit_cost;
...
async function handleWriteDown(args: Record<string, unknown>): Promise<ToolResult> {
  const { id, nrv_per_unit, date, notes } = args as {
    id: string;
    nrv_per_unit: number;
    date: string;
    notes?: string;
  ...
  const body: Record<string, unknown> = { nrv_per_unit, date };
...
async function handleReverseWriteDown(args: Record<string, unknown>): Promise<ToolResult> {
  const { id, new_nrv_per_unit, date, notes } = args as {
    id: string;
    new_nrv_per_unit: number;
    date: string;
    notes?: string;
  };

  const body: Record<string, unknown> = { new_nrv_per_unit, date };
  if (notes) body.notes = notes;
