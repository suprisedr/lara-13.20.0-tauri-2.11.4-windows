...
const HAIRLINE = { style: "thin" as const, color: { argb: COLOR.rule } };
const HAIRLINE_BLUE = { style: "thin" as const, color: { argb: COLOR.emphasis } };
 ...
 */
const WIDTH_TWIPS: Record<string, number> = {
  label: 7725,
  amount: 1815,
  variance: 1815,
  percent: 1150,
 ...
 */
function twipsToWidth(twips: number): number {
  const px = twips / 15;
  return Math.round(((px - 5) / 7) * 100) / 100;
}

// ─── Model types (mirrors ManagementAccountsModelService) ─────────────────────

export type MgmtColumn = {
  kind: "label" | "amount" | "variance" | "percent";
  text: string | null;
  sub?: string;
  current?: boolean;
  group?: string;
...
export type MgmtRow = {
  style: string;
  label: string;
  /** Pre-formatted figures — authoritative for appearance. */
  values: string[];
  /**
   * Live figures, parallel to `values`; null where no figure belongs.
   * Absent entirely on statistics tables, which carry mixed units as text.
   */
  raw?: (number | null)[];
};

export type MgmtStatement = {
  key: string;
  title: string;
  subtitle: string;
  columns: MgmtColumn[];
...
export type MgmtModel = {
  ...
  compare: boolean;
  statements: MgmtStatement[];
  statistics: MgmtStatement[];
  generated_at: string;
};
 ...
 */
function moneyFormat(decimals: number): string {
...
type CellStyle = {
  ...
  size?: number;
  numFmt?: string;
  /** For the header's stacked label and basis. */
  wrap?: boolean;
};

function styleCell(cell: ExcelJS.Cell, s: CellStyle): void {
  cell.font = {
    name: FONT,
 ...
 */
function writeFigure(cell: ExcelJS.Cell, raw: number | null, value: string): void {
...
function writeStatement(
  ...
  const titleLines: Array<[string, number, boolean]> = [
...
function writeCover(workbook: ExcelJS.Workbook, model: MgmtModel): void {
  ...
  const line = (text: string, size: number, bold: boolean, color: string) => {
    const row = sheet.addRow([text]);
    row.height = Math.ceil(size * 1.35);
  ...
  if (model.company.registration_number) field("Registration number", str(model.company.registration_number));
  if (model.company.vat_number) field("VAT number", str(model.company.vat_number));
  if (model.company.income_tax_number) field("Income tax number", str(model.company.income_tax_number));
...
export function buildManagementWorkbook(model: MgmtModel): ExcelJS.Workbook {
  ...
  const taken = new Set<string>(["cover"]);
