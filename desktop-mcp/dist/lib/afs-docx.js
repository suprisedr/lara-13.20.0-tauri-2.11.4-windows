 ...
 */
import { AlignmentType, BorderStyle, Document, LineRuleType, PageOrientation, Paragraph, ShadingType, Table, TableCell, TableLayoutType, TableRow, TextRun, VerticalAlign, WidthType, } from "docx";
// ─── Measured constants ───────────────────────────────────────────────────────
/**
...
function cell(o) {
    ...
    if (o.sub) {
        runs.push(new TextRun({ text: str(o.sub), bold: o.bold ?? false, color: o.color ?? COLOR.rule, font: FONT, size: SIZE.body, break: 1 }));
    }
...
// ─── Header row ───────────────────────────────────────────────────────────────
function headerRow(columns, grid, matrix) {
    return new TableRow({
...
// ─── Body rows ────────────────────────────────────────────────────────────────
function bodyRow(row, columns, grid, matrix) {
 ...
 */
function titleBlock(statement, model) {
    /**
     ...
     */
    const lead = (line, after, before = 0) => ({
        line,
...
function footnotes(statement) {
    return (statement.footnotes ?? []).map((text) => new Paragraph({
 ...
 */
export function buildAfsDocument(model) {
    const children = [];
