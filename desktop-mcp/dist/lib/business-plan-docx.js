...
import { COLOR, FONT, LINE, SIZE, buildDocument, pageBreak, statementTable, str, } from "./afs-docx.js";
// ─── Paragraph helpers ────────────────────────────────────────────────────────
/**
 ...
 */
function lead(line, after, before = 0) {
    return { line, lineRule: LineRuleType.AT_LEAST, before, after };
}
function heading(text, size, color, line, after, before = 0) {
...
function prose(body, muted = false) {
    ...
    return blocks.map((text) => new Paragraph({
...
function coverPage(model) {
    const meta = [];
    if (model.company.company_type)
        meta.push(str(model.company.company_type));
    if (model.company.registration_number)
        meta.push(`Registration number ${model.company.registration_number}`);
    if (model.company.income_tax_number)
        meta.push(`Income tax number ${model.company.income_tax_number}`);
    if (model.company.industry)
        meta.push(str(model.company.industry));
    const children = [
    ...
    if (meta.length > 0) {
        children.push(...meta.map((line) => new Paragraph({
 ...
 */
export function buildBusinessPlanDocument(model) {
    const children = [];
    children.push(...coverPage(model));
    children.push(...contents(model));
    model.sections.forEach((section) => {
        children.push(pageBreak());
        children.push(heading(`${section.number}.  ${str(section.title)}`, SIZE.title, COLOR.navy, LINE.title, 150));
        ...
        model.statistics.forEach((table, index) => {
            children.push(heading(str(table.title), SIZE.sectionHead, COLOR.emphasis, LINE.sectionHead, 75, index === 0 ? 270 : 300));
            if (table.subtitle) {
