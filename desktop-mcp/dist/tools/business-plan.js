...
import { buildBusinessPlanDocument } from "../lib/business-plan-docx.js";
...
export const tools = [
    {
        ...
        description: "Generate a business plan as a Word document (.docx) and save it to the user's Documents folder. " +
        ...
        inputSchema: {
            ...
            properties: {
                ...
                include_statistics: {
                    type: "boolean",
                    description: "Include the derived statistics tables. Defaults to true. Set false for a narrative-only plan.",
                },
                rounding: {
                    type: "number",
                    enum: [1, 1000, 1000000],
                ...
                filename: {
                    type: "string",
                    description: "File name without extension. Defaults to '<company>-business-plan-<year>'. " +
...
export async function handle(name, args) {
    if (name !== "generate_business_plan")
        return null;
    ...
    const model = (await getBusinessPlanModel({
        ...
        includeStatistics: args.include_statistics,
    }));
    ...
    const slug = args.filename ?? `${slugify(model.company.name)}-business-plan-${year}`;
