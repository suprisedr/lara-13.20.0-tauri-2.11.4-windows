...
export const tools = [
    {
        name: "get_leases",
        description: "Fetch IFRS 16 leases on the lease register for a company. " +
    ...
    {
        name: "get_lease",
        description: "Fetch a single IFRS 16 lease by ID, including its current ROU NBV, liability balance, event history, and all terms. " +
            "Requires prior login.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    },
    {
        ...
        description: "Fetch the full amortisation schedule for a lease — one row per month showing payment split (interest vs capital), " +
    ...
    {
        name: "create_lease",
        description: "Recognise a new IFRS 16 lease on the lease register. Supports both lessee and lessor roles.\n" +
    ...
    {
        name: "update_lease",
        description: "Update mutable descriptive fields on a lease (name, asset_tag, category, counterparty, location, notes). " +
    ...
    {
        name: "modify_lease",
        description: "IFRS 16.44–46 Lease Modification — record a change in the scope or consideration of a lease that was not part of the original terms. " +
    ...
    {
        name: "impair_lease",
        description: "IAS 36 Impairment — record an impairment loss on the right-of-use asset when its recoverable amount falls below its carrying amount. " +
    ...
    },
    {
        ...
        description: "IAS 36.114 — reverse a previously recognised impairment on the ROU asset when the recoverable amount has recovered. " +
    ...
    },
    {
        ...
        description: "IFRS 16.B98 Early Termination — derecognise the ROU asset and lease liability when a lease is terminated before its contractual end. " +
...
async function handleGetLeases(args) {
...
async function handleGetLease(args) {
    const { id } = args;
    const data = await apiGet(`/leases/${id}`);
    return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
}
async function handleGetLeaseSchedule(args) {
    const { id } = args;
    const data = await apiGet(`/leases/${id}/schedule`);
    return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
}
async function handleCreateLease(args) {
    const body = {
...
async function handleUpdateLease(args) {
    const { id } = args;
    const body = {};
    for (const k of ["name", "asset_tag", "category", "counterparty", "location", "notes"]) {
        if (args[k] !== undefined)
            body[k] = args[k];
    }
...
async function handleModifyLease(args) {
    const { id, event_date, new_end_date, new_monthly_payment, description } = args;
    const body = { event_date, new_end_date, new_monthly_payment };
    if (description)
        body.description = description;
    const result = await apiPost(`/leases/${id}/modify`, body);
    return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
}
async function handleImpairLease(args) {
    const { id, amount, date, reason } = args;
...
async function handleReverseLeaseImpairment(args) {
    const { id, amount, date } = args;
    const result = await apiPost(`/leases/${id}/reverse-impairment`, { amount, date });
    return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
}
async function handleTerminateLease(args) {
    const { id, termination_date, description } = args;
    const body = { termination_date };
    if (description)
        body.description = description;
