...
export const tools = [
    {
        ...
        description: "Fetch IAS 41 biological assets for a company. " +
    ...
    },
    {
        ...
        description: "Fetch a single IAS 41 biological asset by ID with full details. Requires prior login.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    },
    {
        ...
        description: "Register a new IAS 41 biological asset. " +
    ...
    },
    {
        ...
        description: "Update mutable fields on an IAS 41 biological asset (name, reference, location, quantity, unit, notes). " +
            "Requires prior login." +
            FLAG_RULE,
        inputSchema: {
            type: "object",
    ...
    },
    {
        ...
        description: "IAS 41.12 Fair Value Adjustment — re-measure a biological asset at fair value less costs to sell. " +
    ...
    },
    {
        ...
        description: "IAS 41.13 Harvest — record the harvesting of agricultural produce from a biological asset. " +
    ...
    },
    {
        ...
        description: "IAS 41.27 Disposal — dispose of a biological asset on sale or retirement. " +
...
async function handleFairValueAdjust(args) {
    const { id, ...body } = args;
    const result = await apiPost(`/biological-assets/${id}/fair-value-adjust`, body);
    return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
}
async function handleHarvest(args) {
    const { id, ...body } = args;
    const result = await apiPost(`/biological-assets/${id}/harvest`, body);
    return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
}
async function handleDispose(args) {
    const { id, ...body } = args;
    const result = await apiPost(`/biological-assets/${id}/dispose`, body);
    return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
}
