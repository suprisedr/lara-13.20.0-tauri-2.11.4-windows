...
export const tools = [
    {
        name: "quotations",
        description: "Search quotations for a company. At least one filter must be provided. Requires prior login.\n" +
            "\n" +
...
export async function handle(name, args) {
    if (name !== "quotations")
        return null;
    const { company_id, quotation_number, customer_name, date, status, total_min, total_max } = args;
    ...
    const data = (await graphqlRequest(`query SearchQuotations(
      $companyId: ID!
